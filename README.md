...
4. 오라클 데이터 베이스는 관리자 계정(via SQL developer or sqlplus)에서 아래 처럼 사용자 계정을 생성한 후, 사용자 계정으로 로그인 하여 "Devlog 테스트용 sql문.sql" SQL 스크립트를 실행하여 각 테이블을 생성하고, 테스트용 dummy data를 삽입하여야 한다.<br>

   - [사용자 your_user_name 계정 생성 구문] <br>
